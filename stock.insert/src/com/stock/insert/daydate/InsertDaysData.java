...
/**
 * 本类用来读取文件夹中的文本文件获取的数据插入到数据库中
 * @author 辉
 *
 */
 public class InsertDaysData {
	static Connection con = JdbcUtils.getMysqlCon("stock");

	public static void main(String[] args) {
		InsertDaysData insert = new InsertDaysData();
		//参数是：数据所在的目录和保存数据的表名称
		insert.iteratorTXTPath("D:\\stock\\data","stock_2017");
	}


	/**
	 * 处理字符串拼接成sql语句并调用插入方法插入到数据库中（上面的方法是公用的。只有这里的业务逻辑要重新写）
	 * @param 需要处理的字符串
	 */
	String code[] = null;
	StringBuffer sql = new StringBuffer();
	ArrayList<String> endPrice = new ArrayList<String>();
	public void dealString(String str,String insertTableName){
		if(str.indexOf("日期") == -1 && str.indexOf("通达信") == -1 ){  //去除表头和数据来源
			...
			if(endPrice.size() == 2){
				sql.append("INSERT INTO "+insertTableName+"(securityCode,securityName,date,startPrice,maxPrice,minPrice,endPrice,amount,sumMoney,ZhangFu,ZhenFu) VALUES('"+code[0]+"','"+securityName+"'");
			}else{
	...
	/**
	 * @param path 文件夹所在的路径
	 * 读取传入的地址里面、所有.txt结尾的文件、再读取里面的数据插入到数据库中
	 * @param insertTableName 需要插入的表名称
	 */
	public void iteratorTXTPath(String path,String insertTableName){
		...
		for(File txtPath:txtPaths){ //遍历文本文件
			try {
				...
				while(str != null){  //只要不为空就继续读取下一行并赋值给变量str
					dealString(str,insertTableName);  //调用方法拼接成sql     System.out.println(str);
